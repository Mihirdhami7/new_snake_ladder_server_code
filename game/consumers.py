...
class SnakeLadderConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            userid = text_data_json.get('userid', None)
            move = text_data_json.get('move', None)

            # Broadcast the userid and move to the room group
            if userid and move:
                if move == "no move":
                ...
                logger.info(f"Message broadcasted: userid={userid}, move={move}")
            else:
    ...
    async def player_move(self, event):
        ...
        logger.info(f"Message sent to WebSocket: userid={userid}, move={move}")
    ...
    @database_sync_to_async
    def get_all_users_in_room(self, room_group_name):
        return room_user_ids.get(room_group_name, [])
